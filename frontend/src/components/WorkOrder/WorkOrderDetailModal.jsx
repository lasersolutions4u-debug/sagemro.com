...
const COPY = {
  en: {
    ...
    ratingSaved: 'Service confirmed. Your review helps SAGEMRO match better engineers. Thank you.',
  ...
  cn: {
    ...
    incomplete: '工单信息不完整',
    ratingSaved: '服务已确认。你的评价有助于 SAGEMRO 匹配更合适的工程师，感谢你的反馈。',
    ratingFailed: '评价提交失败：',
...
export function WorkOrderDetailContent({
  ...
  const handleSubmitRating = async () => {
    ...
    if ((!detail?.engineer_id && !businessService) || !detail?.customer_id) { toastWarning(copy.incomplete); return; }
    setSubmitting(true);
    try {
      await submitRating({
      ...
      toastSuccess(businessService ? (isCnLocale() ? '服务报告已验收，尾款另行确认。' : 'Service report accepted. Final payment is tracked separately.') : copy.ratingSaved);
      onRateSuccess?.();
      loadDetail();
    } catch (e) {
  ...
  const captureConversionLocation = async () => {
    ...
    try {
      ...
      toastError(isBrowserGeolocationError(error)
        ? formatGeolocationError(error, isCn)
        : (error.message || (isCn ? '无法获取当前位置。' : 'Unable to get current location.')));
    } finally {
  ...
  if (isCustomer && (canRate || businessAccepted || (effectiveStatus === 'completed' && hasRating))) {
    tabs.push({ key: 'rating', label: isBusinessService ? (isCnLocale() ? '服务验收' : 'Service acceptance') : copy.tabs.review });
  }
  ...
  const renderInfoTab = () => (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-[var(--color-border)] pb-3 text-sm font-semibold text-[var(--color-text-primary)]">
          <span>{(() => {
            ...
            if (['resolved', 'pending_review', 'completed'].includes(serviceState)) {
              return isCn ? '服务已完成' : 'Service complete';
            }
            if (serviceState === 'in_service') return isCn ? '服务进行中' : 'Service in progress';
            return isCn ? '服务待开始' : 'Service pending';
          })()}</span>
          ...
          <span className={quoteExecution.financially_settled ? 'text-green-600' : 'text-amber-600'}>
            {quoteExecution.financially_settled
              ? (isCn ? '款项已结清' : 'Payment settled')
              : (isCn ? '待收尾款' : 'Payment outstanding')}
          </span>
        </div>
      )}
      ...
      {isCustomer && canRate && !hasRating && (
        ...
        >
          {isBusinessService ? (isCnLocale() ? '验收服务报告' : 'Accept service report') : copy.confirmReview}
        </button>
      )}
