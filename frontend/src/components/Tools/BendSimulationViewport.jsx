...
const COPY = {
  en: {
    ...
    fit: 'Fit viewport', reset: 'Reset view', machine: 'Machine', upperTool: 'Upper tool', lowerTool: 'Lower tool', material: 'Material sheet',
  ...
  zh: {
    ...
    fit: '适配视图', reset: '重置视图', machine: '折弯机', upperTool: '上模', lowerTool: '下模', material: '材料板材',
...
export function BendSimulationViewport({ result, activeFrame = 0, viewMode = '2d', onViewModeChange, locale = 'en' }) {
  ...
  const [fitMode, setFitMode] = useState('reset');
  const model = buildBendSimulationViewportModel(result, activeFrame, viewMode);
  ...
  const { flat, formed, tooling, projection } = model;
  const is3d = viewMode === '3d';
  const viewBox = selectBendSimulationViewportViewBox(model, fitMode);
  ...
  return (
    <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-elevated)] p-4 sm:p-5" aria-label={copy.title}>
      ...
      <div className="mt-4 overflow-hidden rounded-lg border border-[var(--color-border)] bg-[#111820] p-2">
        <svg viewBox={viewBox} className="h-64 w-full" role="img" aria-label={`${copy.title}: ${activeDescription}`}>
          {is3d ? (
