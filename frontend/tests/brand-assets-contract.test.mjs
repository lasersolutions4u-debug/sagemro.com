...
test('main site first-impression copy keeps CN and COM market language separate', () => {
  ...
  assert.match(engineerRecruiting, /SAGEMRO 工程师合作网络/);
  assert.match(engineerRecruiting, /SAGEMRO Engineer Partner Network/);
  assert.match(engineerRecruiting, /让专业工程师价值最大化/);
  assert.match(engineerRecruiting, /Maximize the Value of Professional Engineers/);
...
test('engineer recruiting page presents the approved partnership story and application flow', () => {
  ...
  assert.match(recruiting, /networkLabel: '工业设备服务网络'/);
  assert.match(recruiting, /networkLabel: 'Industrial Service Network'/);
  assert.match(recruiting, /让专业工程师价值最大化/);
  assert.match(recruiting, /Maximize the Value of Professional Engineers/);
  assert.match(recruiting, /少处理琐事，多专注有价值的现场服务/);
  assert.match(recruiting, /Spend less time on administration and more on valuable field service/);
  assert.match(recruiting, /全国共享客服中心/);
  assert.match(recruiting, /配件集采与供应链/);
  assert.match(recruiting, /新媒体营销与获客/);
  assert.match(recruiting, /AI 与知识库运营/);
  assert.match(recruiting, /着眼长期服务和共同成长/);
  assert.match(recruiting, /公平、诚信、透明/);
  assert.match(recruiting, /尊重数据价值/);
  assert.match(recruiting, /Submit Service Interest/);
...
test('engineer metric overview keeps eight cards in a two-column mobile grid', () => {
  const overview = read('frontend/src/components/Engineer/EngineerMetricOverview.jsx');

  assert.match(overview, /const METRICS = \[/);
  for (const key of ['needsAction', 'todayTasks', 'pendingConfirmation', 'inService', 'quotePending', 'scheduledDates', 'reportsDue', 'partsNeeds']) {
    assert.match(overview, new RegExp(`'${key}'`));
  }
  assert.match(overview, /grid grid-cols-2 gap-2 lg:grid-cols-4/);
  assert.match(overview, /personal: 'My metrics'/);
  assert.match(overview, /team: 'Team metrics'/);
});

test('engineer workspace delegates English next steps and selected task context', () => {
  ...
  assert.match(detail, /customer: 'Customer'/);
  assert.match(detail, /machine: 'Machine \/ service type'/);
  assert.match(detail, /preparation: 'Job Preparation'/);
  assert.match(detail, /checklist: 'Service Standard Checklist'/);
...
test('engineer workspace keeps metrics and calendar compact above independent work-order lists', () => {
  const workspace = read('frontend/src/components/Engineer/EngineerWorkspace.jsx');
...
test('customer service views translate machine fields to English', () => {
  ...
  assert.match(detailModal, /label: isBusinessService \? \(isCnLocale\(\) \? '服务验收' : 'Service acceptance'\) : copy\.tabs\.review/);
  assert.match(detailModal, /tabs\.push\(\{ key: 'repairRecord', label: 'Service Report' \}\)/);
  assert.match(myServices, /formatCustomerDeviceLine\(order, isCn/);
