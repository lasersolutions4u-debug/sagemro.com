...
test('main site first-impression copy keeps CN and COM market language separate', () => {
  ...
  assert.match(engineerRecruiting, /客服工程师品牌共创，让技术价值充分体现/);
  assert.match(engineerRecruiting, /Co-create the service brand\. Earn what your skill is worth\./);
...
test('engineer recruiting page presents the approved partnership story and application flow', () => {
  ...
  assert.match(recruiting, /networkLabel: '客服工程师品牌共创平台'/);
  assert.match(recruiting, /networkLabel: 'Service Engineer Brand Program'/);
  assert.match(recruiting, /title: '客服工程师品牌共创，让技术价值充分体现'/);
  assert.match(recruiting, /title: 'Co-create the service brand\. Earn what your skill is worth\.'/);
  assert.match(recruiting, /琐碎的交给平台，值钱的留给你/);
  assert.match(recruiting, /Hand over the overhead\. Keep the work that pays\./);
  assert.match(recruiting, /platformSupport: \['品牌授权', '管理系统', '小程序与网站', 'AI 工作助理'\]/);
  assert.match(recruiting, /platformSupportTitle: '平台提供'/);
  assert.match(recruiting, /统一品牌宣传/);
  assert.match(recruiting, /视频、公众号与搜索引擎推广/);
  assert.match(recruiting, /供应链与备件管理/);
  assert.match(recruiting, /服务知识库建设/);
  assert.match(recruiting, /合作制而非雇佣制/);
  assert.match(recruiting, /账目透明，多劳多得/);
  assert.match(recruiting, /第一年免加盟费/);
  assert.match(recruiting, /question: '客户为什么选择我？'/);
  assert.match(recruiting, /question: 'Why would customers choose me\?'/);
  assert.match(recruiting, /Apply to Co-create/);
