...
test('main site first-impression copy keeps CN and COM market language separate', () => {
  ...
  assert.match(engineerRecruiting, /SAGEMRO 工程师合作平台/);
  assert.match(engineerRecruiting, /SAGEMRO Engineer Partner Platform/);
  assert.match(engineerRecruiting, /免费品牌与系统，工程师只专注技术/);
  assert.match(engineerRecruiting, /A free brand licence and free systems, so engineers focus on the work/);
...
test('engineer recruiting page presents the approved partnership story and application flow', () => {
  ...
  assert.match(recruiting, /networkLabel: '工业设备服务合作网络'/);
  assert.match(recruiting, /networkLabel: 'Industrial Service Partner Network'/);
  assert.match(recruiting, /title: '免费品牌与系统，工程师只专注技术'/);
  assert.match(recruiting, /title: 'A free brand licence and free systems, so engineers focus on the work'/);
  assert.match(recruiting, /把品牌、系统、推广和管理交给我们，你专注现场技术/);
  assert.match(recruiting, /Hand over brand, systems, promotion and management, and focus on the work on site/);
  ...
  assert.match(recruiting, /合作制平台/);
  assert.match(recruiting, /公平透明，多劳多得/);
  assert.match(recruiting, /第一年免加盟费/);
  assert.match(recruiting, /AI 的目标只有一个：把专业回复做到越来越靠谱/);
  assert.match(recruiting, /One goal for AI: increasingly reliable professional answers/);
  assert.match(recruiting, /Submit Cooperation Interest/);
