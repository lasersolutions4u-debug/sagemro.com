...
test('CN recruiting and shared overlays localize secondary labels', () => {
  ...
  assert.match(recruiting, /面向激光切割机及金属成形设备行业/);
  assert.match(recruiting, /primary: '提交服务意向'/);
  assert.match(recruiting, /工程师最关心的三个问题/);
  ...
  assert.match(recruiting, /服务代表会收到 SAGEMRO 发出的账号激活链接/);
  assert.doesNotMatch(activation, /认证服务代表/);
