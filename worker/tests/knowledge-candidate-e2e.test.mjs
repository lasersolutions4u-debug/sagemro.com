...
function createE2eEnv() {
  ...
  DB.sqlite.exec(`
    ...
    `).join('\n')}
    ${WORK_ORDERS.flatMap(([id]) => ['resolve', 'handover'].map((gate) => `
      INSERT INTO work_order_service_gate_overrides (
        id, work_order_id, gate_key, reason, overridden_by
      ) VALUES (
        ${sqlText(`fixture-${id}-${gate}`)}, ${sqlText(id)}, ${sqlText(gate)},
        'Fictional E2E fixture isolates the reviewed-knowledge workflow.', ${sqlText(ADMIN_ID)}
      );
    `)).join('\n')}
  `);
  return {
