...
test('046 migration extends repair records with evidence and lifecycle fields', async () => {
...
test('046 migration registers its version in the migration ledger', async () => {
  const sql = await readFile(migrationUrl, 'utf8');
...
test('fresh schema snapshot defines repair records with 016 and 046 fields', async () => {
...
test('046 executes on a 016 baseline and D1 enforces source consistency', () => {
...
function createRepairRecordEnv({
  ...
  function createStatement(sql) {
    const statement = {
      ...
      async first() {
        if (/SELECT (?:id, )?status, engineer_id(?:, customer_id, service_mode, active_quote_version, arrival_verification_required, arrival_verified_at)? FROM work_orders WHERE id = \?/i.test(this.sql)) {
          return {
            id: state.workOrder.id,
            status: state.workOrder.status,
            engineer_id: state.workOrder.engineer_id,
            customer_id: state.workOrder.customer_id,
            service_mode: 'remote',
      ...
      async all() {
        if (/FROM work_order_service_gate_overrides/i.test(this.sql)) {
          return {
            results: [{
              gate_key: 'resolve',
              reason: 'Repair-record fixture isolates final report validation.',
...
test('resolve leaves the work order unchanged when the report disappears before finalization', async () => {
  ...
  assert.deepEqual(state.batchResults.slice(0, 2).map((item) => item.meta.changes), [0, 0]);
  assert.equal(state.workOrder.status, 'in_service');
