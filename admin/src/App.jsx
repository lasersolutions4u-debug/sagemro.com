...
// 后台已裁剪为「知识库中枢」：登录 / 用户统计 / 注册用户管理 / 知识库 / 知识候选 / 内部员工账号。
...
export default function App() {
  ...
  const visibleNavItems = useMemo(() => {
    ...
    // 非 admin 的内部员工（商务 / 运营 / 仓储 / 采购）只保留知识库上传与维护。
    if (user.staffRole !== 'admin') return NAV_ITEMS.filter((item) => item.key === 'knowledge');
    return NAV_ITEMS.filter((item) => (item.key === 'staffAccounts' ? isBootstrapAdmin : true));
  }, [user]);
