...
你正在维护 SAGEMRO 平台（sagemro.com / sagemro.cn）——一个面向钣金加工行业设备后服务市场的 B2B 信息技术服务平台。

## 架构概览

```
国际版前端（React 18 + Vite + Tailwind CSS）
  ↓ Cloudflare Pages: sagemro.com
中国版前端 / 工程师端（同一套 frontend 构建）
  ↓ 阿里云 ECS + nginx: sagemro.cn / engineer.sagemro.cn
Admin（独立前端应用）
  ↓ Cloudflare Pages: admin.sagemro.com
  ↓ 阿里云 ECS + nginx: admin.sagemro.cn
Worker（Cloudflare Workers）
  ↓
D1 数据库（SQLite — 国际库 sagemro-db，中国库 sagemro-db-cn；schema 文件在 worker/migrations/）
  ↓
OpenAI API（通过 api.jiekou.ai 中转 — 模型 gpt-4o-mini）
```

管理后台 `admin/` 是一个独立应用。国际版发布到 `admin.sagemro.com`，中国版发布到 `admin.sagemro.cn`。中国版 `.cn` 真实生产流量走阿里云 ECS，不以 Cloudflare Pages 成功作为最终上线判断。
...
| Cloudflare CI/CD | `.github/workflows/deploy.yml` |
| 中国版 ECS CI/CD | `.github/workflows/aliyun-cn-deploy.yml` |
...
- 跑 `cd frontend && npm test` 确认契约测试和本地化测试通过
- 改布局时必须检查深色模式和移动端适配
- 确保 `App.jsx` 的 prop 传递链路完整
- 中国版改动要检查二级页面、弹窗、admin、engineer、customer 是否仍有英文残留
...
- **国际版自动部署**：push 到 `main` 触发 `.github/workflows/deploy.yml`，部署 `sagemro.com`、`admin.sagemro.com` 和 Worker。
- **中国版 Cloudflare 辅助部署**：push 到 `china-edition` 触发 `.github/workflows/deploy.yml`，部署 Cloudflare Pages 的中国版前台/后台辅助目标，不部署 Worker，也不代表 `.cn` ECS 生产已更新。
- **中国版 ECS 生产部署**：push `china-edition` 后，手动触发 `gh workflow run aliyun-cn-deploy.yml --ref china-edition`，发布 `sagemro.cn`、`admin.sagemro.cn`、`engineer.sagemro.cn`。
- **Worker/API/schema 变更**：需要同步到 `main` 并部署 Worker；`china-edition` 不部署 Worker。新增 D1 migration 后，必须先手动确认生产 D1 已应用。
- **密钥管理**：通过 GitHub Secrets、Cloudflare Worker secrets 或服务器环境配置注入，绝不出现在代码或配置文件中。
