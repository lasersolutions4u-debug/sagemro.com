...
| `CLOUDFLARE_D1_BACKUP_API_TOKEN` | 单独创建的备份 token；Account Resources 只包含 SAGEMRO 账号，按下述导出权限验证授予最小 D1 权限，不要复用部署 token |
| `CLOUDFLARE_ACCOUNT_ID` | CF Dashboard 右侧栏 |

Cloudflare 提供 account-level `D1 Read` 权限，但当前官方 OpenAPI 只明确把该权限列在 list/get/query 等操作上，没有为 `POST /d1/database/{database_id}/export` 声明 token permission group。因此先创建仅限 SAGEMRO 账号、仅授予 `D1 Read` 的 token，并用本节本地备份命令做一次 COM/CN 远端导出验证；如果 Cloudflare 返回权限错误，说明当前 export 不支持只读 token，此时只给这个专用 token 升级为 `D1 Edit`/`D1 Write`，不要附加 Workers、Pages、KV、DNS 等权限。`.github/workflows/d1-backup.yml` 只在数据库导出 step 注入 `CLOUDFLARE_D1_BACKUP_API_TOKEN` 和 account ID；安装、校验、上传及清理 step 都不接收 Cloudflare credentials。无论 Cloudflare 当前接受哪一级 D1 权限，都不得复用 `CLOUDFLARE_API_TOKEN`。
...
生产库由 GitHub Actions workflow `Production D1 Backup - COM and CN`（`.github/workflows/d1-backup.yml`）每天 `18:17 UTC`（北京时间次日 `02:17`）计划导出，也可以在 Actions 页面选择该 workflow 后点击 **Run workflow** 手动执行。**定时运行走无审批人的 `production-backup` 环境，只有手动 Run workflow 才走 `production` 审批门禁**：审批等待没有超时限制，一旦把定时任务放回 `production`，卡在门禁上的那个 run 会永久占住并发组 `production-d1-backup`，其后每晚的计划任务都会被逐个取消（2026-08-11 至 2026-09-14 的实际历史就是如此，期间生产 D1 没有任何自动备份）。备份新鲜度由独立 workflow `D1 Backup Freshness Watchdog`（`.github/workflows/backup-freshness-watchdog.yml`）看守：它每天 `21:43 UTC` 检查 `d1-backup.yml` 最近一次成功 run，超过 12 小时没有成功记录就自动开一个标题为 `D1 backup freshness alert` 的 issue 并让自身失败（同时触发 GitHub 对计划任务失败的邮件通知），恢复健康后自动关闭该 issue。它刻意不挂任何 `environment` 门禁，因为"备份静默停掉"正是它要发现的情况：2026-08-11 至 2026-09-14 期间每晚备份都被取消，五周内无人察觉。任何时候都不要给它加审批门禁，否则看守本身也会静默失效。手动核对可在 Actions 页面选择该 workflow 后点击 **Run workflow**，或本地运行 `node worker/scripts/backup-freshness-watchdog.mjs`（需要 `GITHUB_TOKEN` 和 `GITHUB_REPOSITORY`）。备份新鲜度以实际导出完成时间和 artifact 生成时间为准，排队或等待审批不等于已完成备份。本配置使用公开 recipient `age1napv2rt7lm400g8rcz8gg2rjh04cekjs3x5qw920scqg2v2ga4csu50p7l` 在 runner 内加密导出，并在上传前删除明文 SQL。公钥变更合入默认分支并成功运行后，才对新的自动备份生效。任一数据库导出、加密、明文删除、密文校验、manifest 生成或 artifact 上传失败，整个 Action 都会失败；到该次 run 的 `Export production D1 databases` job 中查看第一个失败 step 和 Wrangler 输出，不要在日志中打印 SQL 或私钥内容。
...
```bash
sha256sum --check com-sha256-manifest.txt
# 或中国库 artifact：sha256sum --check cn-sha256-manifest.txt
```
...
至少按季度进行恢复演练：下载 COM/CN artifacts，先验证密文 SHA-256，再使用离线身份解密；创建只声明同名临时数据库的隔离 Wrangler config，然后分别导入对应 SQL，例如 `npx wrangler d1 execute sagemro-db --local --config /path/to/drill-com.toml --file /path/to/sagemro-db-<timestamp>.sql`，中国库使用声明 `sagemro-db-cn` 的另一个临时 config；执行 schema 探针和关键表计数/抽样业务检查；记录 artifact 名、校验结果、恢复耗时和检查结果；演练后安全删除解密 SQL、本地 D1 state、临时 config 和下载的生产数据。也可以使用访问受限的专用非生产 D1，但不得复用生产 database ID。禁止直接把定时备份导入生产。真正生产恢复前仍需再次导出当前库、安排停写窗口，并先在隔离 D1 验证目标备份。
