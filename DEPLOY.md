...
中国版阿里云 ECS 部署还需要这些 GitHub Actions secrets。它们只用于 `.github/workflows/aliyun-cn-deploy.yml` 手动部署 workflow，不用于 Cloudflare 部署：
...
阿里云 RAM 用户建议单独创建，专用于 GitHub Actions 部署，权限只给 ECS 安全组入方向规则管理。不要使用主账号 AccessKey。
...
资源范围应尽量限制到当前 ECS 安全组。如果控制台自定义策略不熟，先用更宽一点的 ECS 安全组管理权限跑通，再收窄到指定安全组。
