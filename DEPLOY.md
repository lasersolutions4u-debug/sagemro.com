...
macOS 可使用 `shasum -a 256 -c com-sha256-manifest.txt` 或对应的 CN manifest。该 artifact 内的 SQL 必须显示 `OK`；校验失败的文件不得用于恢复。
